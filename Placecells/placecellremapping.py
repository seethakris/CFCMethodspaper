...
class GetData(object):
    def __init__(self, animalinfo, taskdict, FolderName):
        ...
        self.trackbins = 5
        self.animalname = animalinfo['animalname']
    ...
    def get_lapframes_numcells(self):
        ...
        self.numcells = scipy.io.loadmat(os.path.join(self.FolderName, 'Behavior', self.PlaceFieldData[0]))['Allbinned_F'].shape[1]
        print(self.numcells)
    ...
    def calculate_remapping_within_task(self, taskA):
        pc_activity_dict = {keys: [] for keys in [taskA+'A', taskA+'B']}
        cells_to_plot = list(self.sig_PFs_cellnum_revised[taskA])
        for i in self.PlaceFieldData:
            ft = i.find('Task')
            taskname = i[ft:ft + i[ft:].find('_')]
            if taskname == taskA:
                ...
                pc_activity2 = []
                for n, c in enumerate(cells_to_plot):
                    laps = int(np.size(x['Allbinned_F'][0, c], 1)/2)
                    pc_activity1.append(np.nanmean((x['Allbinned_F'][0, c][:, :laps]), 1))
                    pc_activity2.append(np.nanmean((x['Allbinned_F'][0, c][:, laps:]), 1))
                pc_activity_dict[taskA+'A'] = np.asarray(pc_activity1)
                pc_activity_dict[taskA+'B'] = np.asarray(pc_activity2)

        self.save_pcs(pc_activity_dict, 'pcs_sortedby_sametask%s' %taskA)
        ...
        for t in self.PlaceFieldData:
            ...
            for n in np.arange(np.size(self.sig_PFs_cellnum_revised[taskname])):
                ...
                for p1 in np.arange(beg, self.numPFs_incells_revised[taskname][n]):
                    ...
                    self.pfparams_df.loc[count] = {'Task': taskname,
                                        'CellNumber':
    ...
    def save_analyseddata(self):
        ...
        np.savez(os.path.join(self.SaveFolder, f'%s_placecell_data.npz' % self.animalname),
                 ...
                 animalname=self.animalname, numcells=self.numcells)
